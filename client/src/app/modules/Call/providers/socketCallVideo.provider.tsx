...
type TSocketCallVideoEvent = {
  handleEventCall: {
    emitCall: (args: TSocketEventCall) => void;
    emitRejectCall: (args: TSocketEventCall) => void;
    setInstanceHookCall: React.Dispatch<
      SetStateAction<ReturnType<typeof useCall> | undefined>
    >;
    // onAcceptCall: (args: TSocketEventCall) => void;
...
export const SocketCallVideoContext = createContext<
  TSocketCallVideo &
    TSocketCallVideoEvent & {
      instanceHookCall?: ReturnType<typeof useCall> | undefined;
      infoCall: TSocketEventCall | undefined;
    }
>({
  infoUserCall: undefined,
  instanceHookCall: undefined,
  infoCall: undefined,
  handleEventCall: {
    emitCall: (args: TSocketEventCall) => {},
    emitRejectCall: (args: TSocketEventCall) => {},
    setInstanceHookCall: () => {},
    // onAcceptCall: (args: TSocketEventCall) => {},
...
const SocketCallVideoProvider = ({
  ...
  const [instanceHookCall, setInstanceHookCall] = useState<
    ReturnType<typeof useCall> | undefined
  >(undefined);
  ...
  const { onCall, peerReady, onAcceptCallTrigger } = instanceUseCall;
  ...
  const emitAccpetCall = useCallback(
    (args: TSocketEventCall) => {
      ...
      const url = `/call?caller_id=${caller_id}&receiver_id=${receiver_id}&onwer_id=${receiver_id}`;
  ...
  useEffect(() => {
    if (!caller_id || !receiver_id || !onwer_id || !socket) return;
    if (peerReady && socket && onwer_id === caller_id) {
      onCall();
      emitCall({
        caller_id: caller_id,
        receiver_id,
  ...
  return (
    <SocketCallVideoContext.Provider
      value={{
        handleEventCall,
        instanceHookCall: instanceUseCall,
        infoCall,
        infoUserCall,
      }}
    >
